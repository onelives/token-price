...
「我方折扣 × 百炼官网 × 模型原厂」三方 API 价格对比页，由 **GitHub Actions 每天定时抓取官方价格**，自动更新并发布到 GitHub Pages，防止价格信息滞后。

在线地址：<https://onelives.github.io/token-price/>

## 页面结构

- **顶部速览表**：全部模型一表看全，填「我方折扣」自动算我方价；每个价格都可点击跳转对应厂商官方页核价（千问原厂即百炼，标注 `(=百炼)`）。
- **参考明细**：DeepSeek 快照版闲/忙时与均价、三源价格总表、平台折扣规则，位于页面下方按需查阅。
...
价格单位统一为**元/百万 token**（页内可切「亿 token」= ×100）。DeepSeek 原厂为闲/忙分档，对比取**高峰时段**作标准价（保守口径），闲时半价在参考明细中注明。qwen3.7-plus 为长度分档计价，取基础档（输入 ≤256k）。

## 我方折扣

在速览表「我方折扣」列填入折扣（`8` = 8 折，支持小数），我方价 = 百炼标准价 × 折数 ÷ 10，自动计算并随单位切换联动。折数只存在浏览器 localStorage（本机，不上传），自动化不触碰这一列。
